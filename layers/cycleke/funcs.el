...
(defun cycleke/compile-without-debug ()
  ...
  (let (filename suffix progname)
    ...
    (if (string= suffix "c")
        (compile (concat "gcc " filename " -o " progname " -O2 -Wall -lm -std=c11 -static")))
    (if (or (string= suffix "cc") (string= suffix "cpp"))
        (compile (concat "g++ " filename " -o " progname " -O2 -Wall -lm -std=c++14 -static")))
...
(defun cycleke/compile-with-debug ()
  ...
  (let (filename suffix progname)
    ...
    (setq suffix (file-name-extension filename))
    (if (string= suffix "c")
        (compile (concat "gcc " filename " -o " progname " -g -Wall -lm -std=c11 -static")))
    (if (or (string= suffix "cc") (string= suffix "cpp"))
        (compile (concat "g++ " filename " -o " progname " -g -Wall -lm -std=c++14 -static")))
    ))
